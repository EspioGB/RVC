...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        ai_vocals_patha = os.path.join(song_dir, f'{os.path.splitext(os.path.basename(orig_song_path))[0]} ({voice_model} Vocals).{output_format}')
        ...
        ai_vocals_mixed_pathS = add_audio_effects(ai_backing_path, ai_vocals_path, reverb_rm_size, reverb_wet, reverb_dry, reverb_damping)
        ...
        combine_audio([ai_vocals_mixed_path, ai_backing_mixed_path, ai_vocals_mixed_pathS], ai_vocals_patha, main_gain, backup_gain, inst_gain, output_format)

        if not keep_files:
            display_progress('[~] Removing intermediate audio files...', 0.95, is_webui, progress)
            intermediate_files = [vocals_path, main_vocals_path, ai_vocals_mixed_path, ai_backing_mixed_path, ai_vocals_mixed_pathS]
            if pitch_change_all != 0:
        ...
        return ai_vocals_mixed_path, ai_cover_backing_path, ai_vocals_mixed_pathS
